...
public class CustomRadioButton extends JFXRadioButton {
	...
	protected Skin<?> createDefaultSkin() {
		return new CustomRadioButtonSkin(this);
	}
	 ...
	 * @since 2016-03-09
	 */
	private class CustomRadioButtonSkin extends RadioButtonSkin {
		...
		CustomRadioButtonSkin(RadioButton control) {
			super(control);
		...
		private void initializeComponents(final double x, final double y, final double w, final double h) {
			...
			getSkinnable().selectedProperty().addListener((o, oldVal, newVal) -> {
				rippler.setRipplerFill(newVal ? selectedColor : unSelectedColor);
				animation.setRate(newVal ? 1 : -1);
				animation.play();
			});
			animation = new ParallelTransition();
			animation.getChildren().add(new Timeline(
					new KeyFrame(Duration.ZERO,
							new KeyValue(dot.radiusProperty(), minRadius, Interpolator.EASE_BOTH)),
					new KeyFrame(Duration.millis(200),
							new KeyValue(dot.radiusProperty(), radioRadius - 3, Interpolator.EASE_BOTH))));
			animation.getChildren().add(new StrokeTransition(Duration.millis(200), radio, unSelectedColor, selectedColor));
			rippler.setRipplerFill(getSkinnable().isSelected() ? selectedColor : unSelectedColor);
			animation.setRate(getSkinnable().isSelected() ? 1 : -1);
			animation.play();
		}
